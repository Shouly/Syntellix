...
export function AgentInfoSkeleton() {
  return (
    <div className="flex-shrink-0 p-4">
      ...
      <div className="h-3 bg-secondary rounded w-5/6 mb-4"></div>
      <div className="bg-bg-tertiary rounded-lg p-3 mb-4">
        <div className="h-3 bg-secondary rounded w-1/2 mb-2"></div>
        <div className="space-y-1">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="h-2 bg-secondary rounded w-3/4"></div>
          ))}
        </div>
      </div>
    ...
    <>
      ...
      <div className="pb-6 pt-2">
        <div className="relative">
          <div className="w-full h-14 bg-bg-primary rounded-lg border border-secondary"></div>
          <div className="absolute right-4 bottom-3 w-8 h-8 bg-secondary rounded-full"></div>
        </div>
      </div>
    </>
  );
