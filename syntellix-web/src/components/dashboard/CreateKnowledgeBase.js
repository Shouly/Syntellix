...
import { DocumentTextIcon, CloudIcon, PlusIcon, ArchiveBoxIcon, BookOpenIcon, XCircleIcon, ExclamationCircleIcon } from '@heroicons/react/24/solid';
import { CloudArrowUpIcon, DocumentIcon, TableCellsIcon, CodeBracketIcon, PhotoIcon } from '@heroicons/react/24/outline';
import { XMarkIcon } from '@heroicons/react/24/outline';

function CreateKnowledgeBase({ onBack }) {
  ...
  const handleFiles = (uploadedFiles) => {
    ...
    Array.from(uploadedFiles).forEach(file => {
      ...
      if (!isValidType) {
        ...
        newErrors.push(`文件大���超过15MB: ${file.name}`);
      } else {
  ...
  const handleSubmitEmptyKB = (e) => {
    ...
    // 这里添加创建空知识库的逻辑
    console.log('Creating empty knowledge base:', kbName);
    handleCloseModal();
  };
          ...
          <div className="relative top-20 mx-auto p-6 border w-[500px] shadow-lg rounded-2xl bg-white bg-opacity-90">
            ...
            <form onSubmit={handleSubmitEmptyKB} className="space-y-6">
              ...
              <div className="flex justify-end space-x-4">
                ...
                <button
                  type="submit"
                  className="px-8 py-2 font-base bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors duration-200 font-noto-sans-sc"
                >
                  创建
                </button>
              </div>
