...
from syntellix_api.rag.utils.parser_utils import rmSpace
from syntellix_api.rag.nlp import rag_tokenizer, query, is_english
import numpy as np
import logging
...
class Dealer:
    ...
    def search(self, req, idxnm, emb_mdl=None, highlight=False):
        ...
        logger.info("TOTAL: {}".format(self.es.getTotal(res)))
    ...
    def sql_retrieval(self, sql, fetch_size=128, format="json"):
        ...
        logger.info(f"To es: {sql}")

        try:
            tbl = self.es.sql(sql, fetch_size, format)
            return tbl
        except Exception as e:
            logger.error(f"SQL failure: {sql} =>" + str(e))
            return {"error": str(e)}
